...
def read_tsinghua_dataset(image_dir, label_path):
    selected_cat = ['pne', 'p10', 'p5', 'p1', 'pn', 'pl', 'i3', 'i4', 'w32']
    ...
    tsinghua_train_images_path = tsinghua_train_images_path[0]
    tsinghua_val_images_path = utils.read_and_parse_dir(tsinghua_val_images_dir)
    tsinghua_val_images_path = tsinghua_val_images_path[0]
    print('-' * 20)
    ...
    for image_path in tsinghua_train_images_path:
        ...
        for target in targets:
            if target['category'] in selected_cat or 'pl' in target['category']:
...
def read_offline_challenge_dataset(image_dir, label_dir):
    ...
    # train_images_paths = train_images_paths[0]
    val_images_paths = utils.read_and_parse_dir(val_images_dir)
    # val_images_paths = val_images_paths[0]
    print('-' * 25)
    ...
    train_samples_lst = []
    val_samples_lst = []
    for image_dir_path in train_images_paths:
        for image_path in image_dir_path:
    ...
    for image_dir_path in val_images_paths:
        for image_path in image_dir_path:
            video_name = image_path.split('/')[-2]
            frame_name = image_path.split('/')[-1]
            frame_name = frame_name.split('.')[0]
...
def main():
    ...
    train_samples = merge_dataset([tsinghua_train_samples, offline_train_samples,
                                   tsinghua_val_samples, offline_val_samples])
    # val_samples = merge_dataset([tsinghua_val_samples, offline_val_samples])
    create_tf_record('./data/full_tfrecord_2018_one_category/train/train.record', train_samples, label_map_dict)
