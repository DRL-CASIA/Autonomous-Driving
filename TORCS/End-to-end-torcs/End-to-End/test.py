...
train_logs_path = 'C:\\Users\\Chen\\Desktop\\New\\logs\\train'
test_logs_path = 'C:\\Users\\Chen\\Desktop\\New\\logs\\test'

train_writer = tf.summary.FileWriter(train_logs_path,
                                      sess.graph)
test_writer = tf.summary.FileWriter(test_logs_path)
...
#      loss_value = loss.eval(feed_dict={model.x:xs, model.y_: ys, model.keep_prob: 1.0})
      loss_value = loss.eval(feed_dict={model.x: xs, model.y_: ys, model.keep_prob: 1.0})
      summary0=tf.Summary(value=[tf.Summary.Value(tag="loss", simple_value=loss_value)])
      #eval函数其实就是一个把代码变成语句的函数，也就是run的意思
      test_writer.add_summary(summary0, epoch * driving_data_test.num_images/batch_size + i)
      ...
      train_writer.add_summary(summary, epoch * driving_data_test.num_images/batch_size + i)
