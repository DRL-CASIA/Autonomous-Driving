...
void vision_update(){
	...
	for (int i = 0; i < 3*image_height*image_width; i++){
				ReInfo->vision->img[i] = (unsigned char) img[i];
	}
}
...
ReOneStep(double deltaTimeIncrement)
{
 ...
 //           			pdata[(h*image_width+w)*3+2] = img[((image_height-h-1)*image_width+w)*3+2];
	...
	/////////////////// added by lidong (end)
	// printf("---DEBUG: running ReOneStep()...\n");
...
ReStart(void)
{
    ...
    // printf("---DEBUG: running ReStart()...\n");
    // Set up ReInfo vision struct
    ReInfo->vision = (tRmVisionImg*) malloc( sizeof(tRmVisionImg) );
    ReInfo->vision->width = image_width;
    ReInfo->vision->height = image_height;
    ReInfo->vision->size = image_width * image_height * 3;
    ReInfo->vision->img = (unsigned char*) malloc(ReInfo->vision->size * sizeof(unsigned char));
    if (ReInfo->vision->img == NULL) exit(-1);
    // Avoid zero images
    memset(ReInfo->vision->img,1,ReInfo->vision->size*sizeof(unsigned char));
    // printf("---CKPT: before vision_update\n");
    vision_update();
    // printf("***DEBUG: running ReStart()...\n");
}
