...
// extern float* ptoMarking_L; // used to store player angle
// extern float* ptoMarking_M; // used to store player to_middle
// extern float* ptoMarking_LL;; // used to store player laps
...
{
	if (1) {
		float debug_to_middle = car->_trkPos.toMiddle;
		float debug_angle = RtTrackSideTgAngleL(&(car->_trkPos)) - car->_yaw;
		NORM_PI_PI(debug_angle);
		// printf("angle = %f, to_middle = %f\n", debug_angle, debug_to_middle);
	}


	float lane_width_ = 4;
	float lane_center = 4; // left lane center to track center distance
	float to_track_middle = 2*(car->_trkPos.toMiddle - lane_center) / lane_width_;
	// float dist_to_track_middle = 2*car->_trkPos.toMiddle/(car->_trkPos.seg->width); // factor
    float angle_ghost = RtTrackSideTgAngleL(&(car->_trkPos)) - car->_yaw;
    NORM_PI_PI(angle_ghost);
	// printf("angle: %f, to_middle : %f\n", angle_ghost, to_track_middle);
	// *ptoMarking_L = angle_ghost;
	// *ptoMarking_M = to_track_middle;
	// *ptoMarking_LL = car->_laps;
	float dist_to_track_middle = 2*car->_trkPos.toMiddle/(car->_trkPos.seg->width); // factor
    float dist_to_track_middle_m = car->_trkPos.toMiddle;
	float dist_raced = car->_distRaced + 20;
	float dist_from_start = car->_distFromStartLine;
	float to_middle_distance = car->_trkPos.toMiddle;
    ...
    *pcar_y = car_y;
	...
	// 	usleep(1);
	// }
	// printf("human car_x = %f, car_y = %f\n", car_x, car_y);
	// printf("dist_to_track_middle_m: %.4f, to_middle_distance: %.4f, dist_to_track_middle: %.4f, seg_width: %.4f\n", dist_to_track_middle_m, to_middle_distance, dist_to_track_middle, car->_trkPos.seg->width);
	// printf("dist_raced: %.3f, dist_from_start: %.3f\n", dist_raced, dist_from_start);
	// printf("human laps = %f\n", *ptoMarking_ML);
	// printf("to middle norm= %f, value = %f\n", to_track_middle, (car->_trkPos.toMiddle - lane_center));
	...
	// added by lidong for track parsing (start)
	...
	int N = 2;
	for (int i = 0; i < N; ++i) segptr = segptr->next;
	// if (segptr->type == TR_STR){
 //        printf("Track type: straight\n");
 //    } else if (segptr->type == TR_LFT) {
 //        printf("Track type: left turn\n");
 //    } else if (segptr->type == TR_RGT) {
 //        printf("Track type: right turn\n");
 //    } else {
 //        printf("##### ERROR Track type!\n");
 //    }

	// added by lidong for track parsing (end)
