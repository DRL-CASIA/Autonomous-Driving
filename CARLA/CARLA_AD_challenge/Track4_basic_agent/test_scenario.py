...
python test_scenario.py --scenario OtherLeadingVehicle_1 --reloadWorld --agent PATH_TO_AGENT
    e.g. python test_scenario.py --scenario OtherLeadingVehicle_1 --reloadWorld --agent=${ROOT_SCENARIO_RUNNER}/srunner/challenge/autoagents/DummyAgent.py
         python test_scenario.py --scenario FollowLeadingVehicle_1 --reloadWorld --agent=${ROOT_SCENARIO_RUNNER}/srunner/challenge/autoagents/HumanAgent.py 
...
class ScenarioTrainer(ScenarioRunner):
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        vehicle = random.choice(blueprint_library.filter('vehicle.diamondback.century'))  # vehicle.diamondback.century
        print(vehicle)
        transform = carla.Transform(carla.Location(x=254.0, y=134.5, z=0.2))
        car_in_lane = self.world.try_spawn_actor(vehicle, transform)
        ...
        if self.agent_instance is not None:
            ...
            # self.draw_trajectory(self.agent_instance._global_plan_world_coord, 50)
    ...
    def run_agent(self):
        while self.manager._running:
            ...
            ego_action, waypoints = self.agent_instance()
            self.ego_vehicles[0].apply_control(ego_action)
            # self.world.tick()
            for _ in range(1):  # you can control action frequency here
                self.draw_waypoints(waypoints, 1.0/self.frame_rate)
                spectator = self.world.get_spectator()
                ego_trans = self.ego_vehicles[0].get_transform()
                angle = ego_trans.rotation.yaw
                d = 6.4
                a = math.radians(180+angle)
                location = carla.Location(d * math.cos(a), d * math.sin(a), 3.0) + ego_trans.location
                spectator.set_transform(carla.Transform(location, carla.Rotation(yaw=angle, pitch=-20)))
                self.world.tick()
    ...
    def draw_waypoints(self, waypoints, persistency=-1, vertical_shift=1):
        if waypoints is None or len(waypoints) == 0:
            return
        for position in waypoints:
            location = carla.Location(x=position[0], y=position[1])
            if location.distance(self.ego_vehicles[0].get_location()) > 2:
                self.world.debug.draw_point(location+carla.Location(z=vertical_shift), size=0.1, life_time=persistency)

    def draw_trajectory(self, global_plan_world_coord, persistency=-1, vertical_shift=1):
        for index in range(len(global_plan_world_coord)):
            ...
            self.world.debug.draw_point(location, size=0.2, color=carla.Color(34, 125, 81), life_time=persistency)
